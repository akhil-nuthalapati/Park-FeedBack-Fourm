-- =============================================================================
-- Park Maintenance System — Seed Data (Idempotent)
-- Team Zenith
-- =============================================================================
-- Run AFTER schema.sql, functions.sql, policies.sql, and add_notifications_and_resolution.sql.
-- FULLY IDEMPOTENT: All rows use FIXED UUIDs so ON CONFLICT (id) DO NOTHING
-- prevents duplicates. Safe to re-run after any DB restart.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. PARKS — 13 real GVMC Visakhapatnam parks (Fixed UUIDs)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. VISITS — 30 visits with FIXED UUIDs (idempotent re-runnable)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. FEEDBACK — 16 entries with FIXED UUIDs
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. MAINTENANCE REQUESTS — 12 entries with FIXED UUIDs
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. PROFILES — Commented out: create auth users first via Supabase Dashboard
-- ---------------------------------------------------------------------------
-- IMPORTANT: To create staff accounts:
--   1. Go to Supabase Dashboard → Authentication → Users → Add User
--   2. Use these emails & passwords:
--      super_admin@parkms.gov.in  (Admin@2026)
--      admin@parkms.gov.in        (Admin@2026)
--      officer@parkms.gov.in      (Officer@2026)
--      viewer@parkms.gov.in       (Viewer@2026)
--   3. The handle_new_user trigger (functions.sql) will auto-create profiles.
--   4. Then set the role via: UPDATE profiles SET role = 'SUPER_ADMIN' WHERE email = 'super_admin@parkms.gov.in';
-- ---------------------------------------------------------------------------
